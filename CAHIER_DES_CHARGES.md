...
- Compendium `vaisseaux` complété à 13/13 (12 nouveaux vaisseaux/véhicules du même classeur, en plus de la Convergence) — bouclier/PV/salles manquants dans la source complétés par estimation calibrée quand absents, systématiquement signalés comme tels sur la fiche concernée. Porté à 17 en v0.16.1 (4 vaisseaux d'après les images de l'auteur, caractéristiques estimées).
...
- **v0.15.6** : taux d'une compétence = caractéristique + racial + barème du niveau + max(0, métier + ajustement + malus hors métier **au niveau 0 seulement**), borné à [0, plafond] (règle précisée par l'auteur, 2026-09-26 : le niveau s'ajoute toujours, le racial s'applique toujours, le malus hors métier seulement au niveau 0).
- **v0.15.7** : « Kit de réparation » (équipement, 200c, prix fixé par l'auteur ; icône Foundry `icons/tools/hand/wrench-mechanical-heal-blue.webp`, faute de visuel dans `asset_visuel`) ; correctif MJ `0.15.7-kit-de-reparation` (kits existants, nom sans accents).
- **v0.15.8** : lien vers le vaisseau (onglet Équipements de la fiche classique) — `system.vaisseau { uuid, nom }`, un acteur « vaisseau » du monde déposé sur la fiche devient le vaisseau du personnage (`PersonnageSheet#_onDropActor`, vaisseau de compendium refusé) ; carte image / nom / classe / coque / bouclier qui ouvre la fiche du vaisseau (droit Observateur requis, sinon nom et image seuls + message) ; bouton pour retirer le lien. Onglet Équipements en grille 2 × 2 : Armes | Armures et boucliers, Équipement | Vaisseau. En mode Édition, corbeille sur chaque ligne de l'onglet Équipements (pas dans Combat) → `#supprimerObjet` après confirmation (comme le menu contextuel).
- **v0.16.0** : refonte de la fiche de vaisseau (mockup de l'auteur) — jauges coque / bouclier / réduction, interrupteur de bouclier, armement et postes d'équipage édités par fenêtre (tableau complet réécrit), noms d'équipage par place fusionnés par `VaisseauSheet#_processFormData`, aménagements en pastilles ; schéma : `armement[].emplacement`, `equipage[].places` / `noms[]`, `bouclier.max` (`migrateData`) ; `styles/vaisseau.css` ; traduction `TYPES.Actor.vaisseau`. Mode Édition (identité, maximums, armes, postes, aménagements) ; `amenagements[] { nom, description }` ; `equipage[].uuids[]` (acteurs déposés sur les postes, hors Édition aussi) ; **armement = objets « arme »** du vaisseau (glisser-déposer, drapeau `emplacement`, `Item#estArmeDeVaisseau` : tir au taux du token sélectionné), ancien `system.armement` converti (bouton de la fiche, correctif `0.16.0-armement-en-objets`, sources du compendium) ; `migrateData` ignoré sur les mises à jour partielles. Jauge de réduction du bouclier retirée (champ conservé), déplacement agrandi.
- **v0.16.1** : 4 nouveaux vaisseaux au compendium (Pourparler, Lance d'argent, La Brique, Lumière de l'aube), images de l'auteur réduites dans `asset_visuel/objets/vaisseaux-*` (acteur, portrait, token) ; caractéristiques **estimées** (choix de l'auteur), calibrées sur les vaisseaux existants et signalées dans la description. Images du Frelon, de la Convergence et du Gunboat (correctif `0.16.1-images-vaisseaux` pour les copies du monde).
- **v0.16.2** : bannière pour les 10 compendiums (`system.json` `packs[].banner`, `asset_visuel/objets/banniere-<pack>.jpg`, 600 × 168) — mosaïques de visuels déjà publiés (ethnies, objets, vaisseaux), dessinées dans le navigateur (WebP illisible par System.Drawing).
- **v0.16.3** : descriptions d'ambiance pour les 80 armes, 3 armures et 11 équipements (texte existant conservé en suite) ; nouveaux objets Robe de jedi, Tenue d'apprenti jedi, Robe noire, Tenue de contrebandier (équipements, prix estimés) et Armure de guerrier sith (réduction 4), Plastron blindé (réduction 2) — visuels de l'auteur (`asset_visuel/objets/vetements-*`, `armures-*`) ; visuels de l'Accessoire silencieux et de la Cartouche de carbonite ; tenues de départ reliées (`CORRESPONDANCES` de `objets-depart.mjs`) ; correctif `0.16.3-visuels-descriptions-objets` (image générique / description vide ou ancienne seulement).
- **v0.17.0** : Actor `equipage` (`EquipageData` : `membres[]` = UUID d'acteurs personnage / rapide / sith / PNJ, `vaisseau { uuid, nom }`, `credits`, `portrait`, `description`) et `EquipageSheet` (onglets Membres / Réserve / Notes, caisse et vaisseau dans l'en-tête) ; réserve = objets de l'acteur Équipage, transferts par `helpers/equipage.mjs` (`deplacerObjet` : copie + suppression, droits Propriétaire des deux côtés ; `transfererCredits` avec message) ; hook `dropActorSheetData` : un objet glissé depuis une réserve vers une fiche est déplacé ; lignes d'inventaire de la fiche classique `draggable` ; vaisseau de l'équipage affiché par défaut dans l'onglet Équipements (`vaisseauDEquipage`) ; création : image `icons/environment/people/group.webp`, propriétaire par défaut.
- **v0.18.0** : générique façon intro spatiale (maquette A validée) — JournalEntryPage `generique` (`GeneriqueData` : épisode, titre, ouverture, texte en paragraphes, vitesse, musique, fond ; `PageGeneriqueSheet` : édition / lecture avec « Diffuser le générique » (MJ) et « Aperçu ») ; `apps/generique.mjs` : écran plein écran (canvas d'étoiles, ouverture, logo, défilement en perspective à vitesse proportionnelle à la taille du texte), diffusé par socket `system.galactic-wars` (`lancer` / `passer` / `arreter`), musique par `AudioHelper` chez chaque client, Échap = fermer chez soi ; `styles/generique.css`.
- **v0.18.1** : images du Moto speeder, du Arcadia, du Corellian Dawn, du CEC XS-122 et de La poubelle géante (validées par l'auteur ; `front_speeder.jpg` était un WebP, conversion dans le navigateur) ; correctif `0.18.1-images-vaisseaux` (`vaisseauxSansImage(images)` paramétré).
- **v0.18.2** (suivi de l'auteur, bugs) : `GW.typesTokenLie` (personnage, sith, rapide, vaisseau, équipage) → prototype lié à la création ; correctif `0.18.2-tokens-lies` (sauvegarde de l'acteur dans un dossier, recopie de la fiche du token posé, liaison) ; `GalacticWarsActorDirectory` (glisser les acteurs observés / possédés sans le droit « Créer des tokens ») ; métier Guerrier jedi (brouillon à valider) ; Datapad (200c, estimé) ; tri alphabétique des compétences de l'onglet Combat ; description du personnage 180 / 440 px ; libellé du dépassement de niveau.
