...
- **v0.16.0** : refonte de la fiche de vaisseau (mockup de l'auteur) — jauges coque / bouclier / réduction, interrupteur de bouclier, armement et postes d'équipage édités par fenêtre (tableau complet réécrit), noms d'équipage par place fusionnés par `VaisseauSheet#_processFormData`, aménagements en pastilles ; schéma : `armement[].emplacement`, `equipage[].places` / `noms[]`, `bouclier.max` (`migrateData`) ; `styles/vaisseau.css` ; traduction `TYPES.Actor.vaisseau`.
